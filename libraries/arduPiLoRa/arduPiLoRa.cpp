...
uint8_t SX1272::getHeader()
{
    ...
    if (bitRead(REG_MODEM_CONFIG1, 2) == 0) { // explicit header mode (ON)
        _header = HEADER_ON;
        state = 1;
...
int8_t SX1272::setHeaderOFF()
{
    ...
    if (_modem == FSK) { // header is not available in FSK mode
        ...
        config1 = readRegister(REG_MODEM_CONFIG1);
        if (bitRead(config1, 2) == HEADER_OFF) { // checking headerOFF taking out bit 2 from REG_MODEM_CONFIG1
            state = 0;
            _header = HEADER_OFF;
...
uint8_t SX1272::getCRC()
{
    ...
    if (_modem == LORA) { // LoRa mode
        ...
        value = readRegister(REG_MODEM_CONFIG1);
        if (bitRead(value, 1) == CRC_OFF) { // CRCoff
            _CRC = CRC_OFF;
#if (SX1272_debug_mode > 1)
...
uint8_t SX1272::setCRC_ON()
{
    ...
    if (_modem == LORA) { // LORA mode
        ...
        config1 = readRegister(REG_MODEM_CONFIG1);
        if (bitRead(config1, 1) == CRC_ON) { // take out bit 1 from REG_MODEM_CONFIG1 indicates RxPayloadCrcOn
            state = 0;
            _CRC = CRC_ON;
#if (SX1272_debug_mode > 1)
        ...
        config1 = readRegister(REG_PACKET_CONFIG1);
        if (bitRead(config1, 4) == CRC_ON) { // take out bit 4 from REG_PACKET_CONFIG1 indicates CrcOn
            state = 0;
            _CRC = CRC_ON;
#if (SX1272_debug_mode > 1)
            printf("## CRC has been activated ##\n");
...
uint8_t SX1272::setCRC_OFF()
{
    ...
    if (_modem == LORA) { // LORA mode
        ...
        config1 = readRegister(REG_MODEM_CONFIG1);
        if ((bitRead(config1, 1)) == CRC_OFF) { // take out bit 1 from REG_MODEM_CONFIG1 indicates RxPayloadCrcOn
            state = 0;
            _CRC = CRC_OFF;
#if (SX1272_debug_mode > 1)
...
uint8_t SX1272::setSF(uint8_t spr)
{
    ...
    if (_modem == FSK) {
        ...
        case SF_9:
            ...
            break;
        case SF_10:
            config2 = config2 & 0B10101111; // clears bits 6 & 4 from REG_MODEM_CONFIG2
        ...
        case SF_11:
            ...
            getBW();
            if (_bandwidth == BW_125) { // LowDataRateOptimize (Mandatory with SF_11 if BW_125)
        ...
        case SF_12:
            ...
            if (_bandwidth == BW_125) { // LowDataRateOptimize (Mandatory with SF_12 if BW_125)
                config1 = config1 | 0B00000001;
            }
            break;
        }
...
int8_t SX1272::setBW(uint16_t band)
{
    ...
    case BW_125:
        ...
        getSF();
        if (_spreadingFactor == 11) { // LowDataRateOptimize (Mandatory with BW_125 if SF_11)
            config1 = config1 | 0B00000001;
        }
        if (_spreadingFactor == 12) { // LowDataRateOptimize (Mandatory with BW_125 if SF_12)
            config1 = config1 | 0B00000001;
        }
        break;
    case BW_250:
...
int8_t SX1272::setCR(uint8_t cod)
{
    ...
    switch (cod) {
    case CR_5:
        config1 = config1 & 0B11001111; // clears bits 5 & 4 from REG_MODEM_CONFIG1
        config1 = config1 | 0B00001000; // sets bit 3 from REG_MODEM_CONFIG1
        break;
    case CR_6:
        config1 = config1 & 0B11010111; // clears bits 5 & 3 from REG_MODEM_CONFIG1
    ...
    case CR_7:
        ...
        break;
    case CR_8:
        config1 = config1 & 0B11100111; // clears bits 4 & 3 from REG_MODEM_CONFIG1
    ...
    switch (cod) {
    case CR_5:
        if (((config1 >> 3) & 0B0000111) == 0x01) {
            state = 0;
        }
        break;
    case CR_6:
        if (((config1 >> 3) & 0B0000111) == 0x02) {
            state = 0;
        }
        break;
    case CR_7:
        if (((config1 >> 3) & 0B0000111) == 0x03) {
            state = 0;
        }
        break;
    case CR_8:
        if (((config1 >> 3) & 0B0000111) == 0x04) {
            state = 0;
        }
        break;
    }
...
#endif
            _hreceived = true;
            while ((header == 0) && (millis() - previous < (unsigned long)wait)) { // Waiting to read first payload bytes from packet
...
#endif
        if ((_destination == _nodeAddress) || (_destination == BROADCAST_0)) { // LoRa or FSK mode
            forme = true;
#if (SX1272_debug_mode > 0)
...
int8_t SX1272::getPacket(uint16_t wait)
{
    ...
    if (_modem == LORA) { // LoRa mode
        ...
        if ((bitRead(value, 6) == 1) && (bitRead(value, 5) == 0)) { // packet received & CRC correct
            ...
            if (bitRead(value, 5) != 0) { // CRC incorrect
...
uint8_t SX1272::setTimeout()
{
    ...
    state = 1;
    if (_modem == LORA) {
        switch (_spreadingFactor) { // Choosing Spreading Factor
        case SF_6:
            switch (_bandwidth) { // Choosing bandwidth
            case BW_125:
                switch (_codingRate) { // Choosing coding rate
                case CR_5:
                    _sendTime = 335;
                    break;
                case CR_6:
...
uint8_t SX1272::setPacket(uint8_t dest, char* payload)
{
    ...
    if (_retries == 0) { // Updating this values only if is not going to re-send the last packet
